...
export const productPageEN = {
    ...
    document.getElementById('product-toCart-btn').addEventListener('click',
      async () => {
        ...
        if (chosenSize === 0) {
          ...
          mainMessage.innerText = 'Choose a size first!'
        } else {
          messageWindow.classList.toggle('hidden')
          messageWindow.className = 'message-window-success'
          messageIcon.innerHTML = success
          mainMessage.innerText = 'Product has been added to the cart!'
          chosenSize = 0
        }
      ...
      } else if (status.innerText === 'Currently out of stock') {
  ...
  render: async function () {
    ...
    <div class="md:grid md:grid-cols-2">
        ...
        <div class="flex flex-col gap-8">
            ...
            <div id="" class="flex justify-center text-2xl">Sizes</div>
            ...
            <div class="text-2xl text-center md:text-start">Availability:
              <div id="product-isInStock">${product.isInStock ? 'In stock' : 'Currently out of stock'}</div>
            </div>
            </div>
            <div id="product-toCart">
              <button id="product-toCart-btn" class="bg-green-500 text-white rounded-full mb-8 px-8 py-2 shadow-md hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">Add to cart</button>
            </div>
        </div>
    </div>
  ...
  <div class="relative h-full">
    <img id="full-product-image" src="/uploads/${product.image1}" class="hover:cursor-zoom-in transform transition-transform m-auto max-w-full max-h-full relative z-20">
    <button id="full-pic-close" class="absolute top-0 right-0 z-20 bg-white rounded-full p-2 m-4">Close</button>
  </div>
</div>
    `
  },
