...
export const editProductsPageEN = {
  render: async function () {
    ...
    return `
      <div> 
        <p class="uppercase text-3xl md:text-4xl p-8 flex justify-center mt-16">All products</p> 
      </div>
    ...
    deleteButtons.forEach(button => {
      ...
      button.addEventListener('click', async () => {
        ...
        if (result.status === 'ok') {
          ...
          mainMessage.innerText = 'Product deleted successfully!'

        } else if (result.status === 'Unauthorized') {
          messageWindow.classList.toggle('hidden')
          messageWindow.className = 'message-window-failed'
          messageIcon.innerHTML = failed
          mainMessage.innerText = "You don't have permission for that!"
        }
